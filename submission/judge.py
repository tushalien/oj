...
def runjudge(runid):
	try:
		...
		if "-cache" not in sys.argv: 
			cursor.execute("SELECT submissions.rid as rid,submissions.pid as pid,tid,submissions.language,submissions.name as name,submissions.code as code,error,pro_input,problems.pro_output as output,timelimit FROM submissions,problems WHERE problems.pid=submissions.pid ORDER BY submissions.rid ASC LIMIT 0,1")
			#cursor.execute("SELECT submissions.rid as rid,submissions.pid as pid,tid,submissions.language,submissions.name as name,submissions.code as code,error,pro_input,problems.pro_output as output,timelimit FROM submissions,problems WHERE problems.pid=submissions.pid and submissions.access!='deleted' and submissions.rid = '"+str(runid)+"' and submissions.language in "+str(tuple(languages))+" ORDER BY submissions.rid ASC LIMIT 0,1")
		# else:
		# 	cursor.execute("SELECT submissions1.rid as rid,submissions1.pid as pid,tid,submissions1.language,submissions.name as name,submissions.code as code,error,timelimit FROM submissions AS submissions1,problems, submissions WHERE problems.pid=submissions1.pid and submissions1.rid = submissions.rid and submissions1.access!='deleted' and submissions1.rid = '"+str(runid)+"' and submissions1.language in "+str(tuple(languages))+" ORDER BY submissions1.rid ASC LIMIT 0,1")
		# Select an Unjudged Submission
		run = cursor.fetchone()
		print ("hi")
		print (run)
		print("d")
		cursor.execute("UPDATE submissions SET result='...' WHERE rid='%d'" % (run["rid"]));
		...
		if result==None:
			...
			if "-cache" not in sys.argv: file_write("env/input.txt",run["pro_input"]);
			else:
				try:
		...
		cursor.execute("UPDATE submissions SET time='%.3f',result='%s' WHERE rid=%d" % (float(timetaken),result,int(run["rid"])));
		cursor.execute("UPDATE submissions SET error=\"%s\",output=\"%s\" WHERE rid=%d" %(re.escape(error), re.escape(output),int(run["rid"])));
...
class MyTCPHandler(SocketServer.StreamRequestHandler):
	...
	def handle(self):
		...
		if(self.data == 'rejudge'):
			...
			cursor = link.cursor(sql.cursors.DictCursor)
			cursor.execute("SELECT rid FROM submissions WHERE result is NULL and access != 'deleted'")
			link.close()
			i = 0
